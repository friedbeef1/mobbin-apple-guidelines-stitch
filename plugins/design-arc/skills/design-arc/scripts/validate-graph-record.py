...
def validate_record(record: Any, expected_project_id: str, expected_review_id: str) -> None:
    ...
    for index, raw_edge in enumerate(root["edges"]):
        ...
        provenance_kind = require_nonempty_string(
            edge["provenance_kind"], f"edges[{index}].provenance_kind"
        )
...
def main(argv: list[str]) -> int:
    if len(argv) != 4:
        print(
            "usage: python3 validate-graph-record.py GRAPH_PATH EXPECTED_PROJECT_ID EXPECTED_REVIEW_ID",
            file=sys.stderr,
        )
        return 2
