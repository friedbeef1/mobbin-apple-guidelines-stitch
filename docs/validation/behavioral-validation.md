...
The upgrade harness runs twice with temporary checkouts, projects, and `CODEX_HOME` directories. One baseline is immutable public 0.2.2 commit `1c9b3796e6f5f0648bae5984f1b8e3013eeac56f`; the other is exact local 0.2.3 candidate commit `489988474dce4a4b0da7a5c48104e9d548c107bd`. Each fallback validates the exact baseline plugin identity, source, complete cached tree, and project snapshot before removal, then installs exactly one enabled 0.3.0 plugin whose complete cache equals the candidate package.
