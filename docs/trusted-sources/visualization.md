...
Visualization turns a recommended journey into a concrete proposal that can be inspected across its material entry, transition, loading, empty, error, success, cancellation, and recovery states. Design Arc generates a consolidated static journey board in Codex by default and can use [Google Stitch](https://stitch.withgoogle.com/) as an optional external editing workspace after the direction is established.

## Why there are two routes

Codex is the lower-friction route for a bounded proposal and a few corrections. It generates screens rather than disposable application logic. Stitch becomes useful when sustained visual iteration, multi-screen changes, self-editing, collaboration, future-session continuity, device variants, or design export makes a persistent canvas easier to manage. Design Arc may recommend that move early, but the user always chooses it.
