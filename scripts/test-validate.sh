...
apple_plugin_manifest="$apple_plugin_path/.codex-plugin/plugin.json"
...
[ -f "$apple_plugin_manifest" ] || missing_layout="$missing_layout $apple_plugin_manifest"
...
[ -f "$apple_skill_path" ] || missing_layout="$missing_layout $apple_skill_path"
...
python3 "$codex_skills_dir/.system/plugin-creator/scripts/validate_plugin.py" "$apple_plugin_path"
python3 "$codex_skills_dir/.system/skill-creator/scripts/quick_validate.py" "$(dirname "$apple_skill_path")"

python3 - "$plugin_manifest" "$apple_plugin_manifest" "$marketplace_manifest" <<'PY'
import json
import sys

plugin_path, apple_plugin_path, marketplace_path = sys.argv[1:]
plugin = json.load(open(plugin_path, encoding="utf-8"))
apple_plugin = json.load(open(apple_plugin_path, encoding="utf-8"))
...
for key, expected in expected_apple_plugin.items():
    if apple_plugin.get(key) != expected:
        raise SystemExit(f"apple-guidelines-stitch plugin {key} must be {expected!r}")
if apple_plugin.get("author", {}).get("name") != "James Yeang":
    raise SystemExit("apple-guidelines-stitch plugin author must be James Yeang")
if apple_plugin.get("interface", {}).get("displayName") != "Apple Guidelines + Stitch":
    raise SystemExit("apple-guidelines-stitch display name must be Apple Guidelines + Stitch")
if apple_plugin.get("interface", {}).get("category") != "Productivity":
    raise SystemExit("apple-guidelines-stitch plugin category must be Productivity")
if not isinstance(apple_plugin.get("interface", {}).get("defaultPrompt"), list):
    raise SystemExit("apple-guidelines-stitch defaultPrompt must be a list")
if len(apple_plugin["interface"]["defaultPrompt"]) > 3:
    raise SystemExit("apple-guidelines-stitch defaultPrompt must contain at most three prompts")
for forbidden in ("apps", "mcpServers", "hooks"):
    if forbidden in apple_plugin:
        raise SystemExit(f"apple-guidelines-stitch plugin must not declare {forbidden}")
...
expected_apple_entry = {
    "name": "apple-guidelines-stitch",
    "source": {"source": "local", "path": "./plugins/apple-guidelines-stitch"},
    "policy": {"installation": "AVAILABLE", "authentication": "ON_INSTALL"},
    "category": "Productivity",
}
if marketplace.get("plugins") != [expected_entry, expected_apple_entry]:
    raise SystemExit("marketplace must expose the canonical fb-ux and apple-guidelines-stitch plugin entries")
PY

grep -F 'name: fb-ux' "$skill_path" >/dev/null || fail 'SKILL.md must use the fb-ux internal name'
grep -F 'name: apple-guidelines-stitch' "$apple_skill_path" >/dev/null || fail 'SKILL.md must use the apple-guidelines-stitch internal name'
...
then
  ...
  printf '%s\n' "$validation_output" >&2
  fail 'repository validation failed while checking both package validator coverage'
fi

printf '%s\n' "$validation_output" | grep -F 'PASS: plugin and embedded skill validation for both packages' >/dev/null || {
  printf '%s\n' "$validation_output" >&2
