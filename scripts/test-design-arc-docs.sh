...
require_text "$readme" '**Your part is only the decisions. Design Arc does the work between them.**'
require_text "$readme" '| When | What you do | What Design Arc handles |'
require_text "$readme" '| Every review | Describe the problem and the outcome you want. | Confirm the objective; audit the existing journey; inspect every important state; gather and label evidence; recommend a direction; check platform guidance; specify motion; visualize the complete journey; and validate the proposal. |'
require_text "$readme" '| Once per project | Choose the evidence and approval preferences, and optionally approve a pinned project home. | Save those choices for the project, keep products separate, and reuse the same home on later visits. |'
require_text "$readme" '| Only when requested | Authorize an external source or tool, or approve a direction or visual proposal when your chosen mode pauses. | Prepare the evidence, alternatives, recommendation, trade-offs, complete states, and approval-ready record before asking you. |'
